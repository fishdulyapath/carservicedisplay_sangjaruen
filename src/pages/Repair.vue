<template>
  <div class="withdraw-container">
    ...
    <Dialog
      ...
      modal
      header="ยืนยันดำเนินการเสร็จ"
      :closable="true"
      :style="{ width: '450px' }"
    >
      <div class="confirmation-content">
        <i class="pi pi-exclamation-triangle mr-3" style="font-size: 2rem" />
        <span>คุณต้องการยืนยันดำเนินการเสร็จใช่หรือไม่?</span>
      </div>
      <template #footer>
        <Button
          label="ไม่"
    ...
    >
      <div class="success-content">
        ...
        <span>ดำเนินการเสร็จเรียบร้อยแล้ว</span>
      </div>
      <template #footer>
...
const confirmWithdraw = async () => {
  ...
  try {
    ...
    console.log(times);
    const res = await MasterdataService.updateCarDoc(doc_no.value, "2", times);
    if (res.success) {
